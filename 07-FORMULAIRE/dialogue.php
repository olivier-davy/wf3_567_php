...
if (!empty($_POST)) { // si le formulaire a été envoyé
    ...
    // Pour l'exemple on injecte le CSS : <style>body{display:none}</style>

    // Pour s'en prémunir, nous faisons :
    $_POST['pseudo'] = htmlspecialchars($_POST['pseudo'], ENT_QUOTES);
    $_POST['message'] = htmlspecialchars($_POST['message'], ENT_QUOTES); // cette fonction prédéfinie convertit les caractères spéciaux (<, >, & et les "") en entités HTML (le < devient &lt; le > devient &gt; les guillements deviennent &quot; etc...).
...
<h1>Votre message :</h1>
<form method="post" action=""> <!-- Pour faire simple, on utilise GET pour obtenir des données, et POST pour transmettre des données-->
