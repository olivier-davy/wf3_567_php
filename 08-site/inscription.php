...
if (!empty($_POST)) { // si $_POST n'est pas vide c'est que le formulaire a été envoyé
    ...
    // si le pseudo n'existe pas dans $_POST c'est que le formulaire est altéré, ou si sa valeur est < 4 ou si sa valeur est > 20 (pour la BDD) on met un message d'erreur à l'internaute.
    ...
    //---------------------------
    // S'il n'y a plus d'erreur sur le formulaire, on vérifie si le pseudo existe ou pas avant d'inscire l'internaute en BDD :
...
?>
<h1 class="mt-4">Inscription</h1> <!-- class Bootstrap -->
<?php
...
?>
    ...
    <div><input type="text" name="pseudo" id="pseudo" value="<?php echo $_POST['pseudo'] ?? ''// ne pas efffacer le txt après validation ?>"></div>
    ...
    <div><input type="text" name="code_postal" id="code_postal" maxlength ="5", value="<?php echo $_POST['code_postal'] ?? '' ?>"></div>

    <div><label for="adresse">Adresse</label></div>
    <div><textarea name="adresse" id="adresse" cols="30" rows="10"><?php echo $_POST['adresse'] ?? '' ?></textarea></div><br>
