...
{  // on ne peut pas modifier son propre profil :
	if ($_GET['id_membre'] != $_SESSION['membre']['id_membre']) {
        ...
        // La ternaire ci-dessus est équivalente à la condition if suivante
        //if ($_GET['statut'] == 0){
        //    $statut = 1;
        //} else {
        //    $statut = 0;
...
// 2- Préparation de l'affichage : A RETENIR
...
$contenu .=  '<table class="table">';
		...
		while ($membre = $resultat->fetch(PDO::FETCH_ASSOC)) // boucle car plusieurs membres potentiellement inscrits sur le site
		{
			$contenu .=  '<tr>';
