...
if (!empty($_POST)){ // si le formulaire a été envoyé
   ...
   // 5 suite : traitement de la photo :
    ...
    )); // REPLACE INTO fait un INSERT quand l'ID donné n'existe pas (0). Il se comporte comme un UPDATE quand l'ID donnée existe 
...
//debug($_GET);  
if (isset($_GET['id_produit'])) { // si 'id_produit est dans l'URL, c'est que nous avons demandé la modification du produit : on selectionne les infos du produit en BDD pour l'afficher dans le formulaire
    $resultat = executeRequete("SELECT * FROM produit WHERE id_produit=:id_produit", array
...
?>
<form action="" method="post" enctype="multipart/form-data" > <!--l'attribue enctype spécifie que le formulaire envoie des données binaires (fichier) et du texte (champs de formulaire) : permet d'uploader une photo -->

    <input type="hidden" name="id_produit" value="<?php echo $produit['id_produit']??0;?>"><!-- ce champ est nécessaire pour la modification ou la suppression d'un produit, car on a besoin de l'ID pour la requête SQl. Nous mettons 0 par défaut en value pour que le REPLACE se comporte un INSERT et insère le produit en BDD. type "hidden' pour cacher le champ -->
    ...
    <div><label for="categorie">Catégories</label></div>
    ...
    <div><label for="titre">Description</label></div>
    ...
    <!-- 5- upload de photo   -->    
    <input type="file" name="photo" id="photo"> <!-- on n'oublie pas l'attribut enctype="multipart/form-data" -->
