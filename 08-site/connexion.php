...
$message = ''; // pour afficher le message de connexion
...
if (isset($_GET['action']) && $_GET['action'] == 'deconnexion'){ // si le membre a cliqué "déconnexion"
    ...
    // debug($_SESSION);
    unset($_SESSION['membre']); // on ne fait pas une session_destroy() qui supprimerait toute la session car on veut pouvoir conserver le panier de l'internaute.
    $message = '<div class="alert alert-info">Vous êtes déconnectés.</div>';
...
if (!empty($_POST)) { // si le formulaire a été envoyé
    ...
    if (empty($_POST['pseudo']) || empty($_POST['mdp'])) {  // empty vérifie : '',  NULL, FALSE, non défini.
    // si le champ pseudo ou le champ mdp sont vides ou non définis

        $contenu .= '<div class="alert alert-danger">Les identifiants sont obligatoires.</div>';

    }

    // S'il n'ya pas de message d'erreur à l'internaute, on sélectionne le pseudo en BDD :
    if (empty($contenu)) { // si c'est vide c'est qu'il n'y a pas d'erreur
        ...
        if ($resultat->rowCount() == 1) { // s'il y a 1 ligne de résultat alors le pseudo existe : on peut vérifier le mdp
            ...
            if (password_verify($_POST['mdp'], $membre['mdp'])) {
                // si le mdp du formulaire correspond au hash du mdp alors cette fonction retourne true
...
<form action="" method="post">
    ...
    <div><input type="password" name="mdp" id="mdp"></div><br>
