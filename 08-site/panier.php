...
if(!empty($_POST)){// si le formulaire d'ajout au panier a été envoyé
    ...
    // On remplit la session avec les infos du produit ajoutées au panier (on les met dans la session pour que le panier soit accessible dans toutes les pages du site)
    ...
    $_SESSION['panier']['prix'][] = $produit['prix']; // le prix de la BDD
...
if (empty($_SESSION['panier']['id_produit'])){ // s'il n'ya pas d'id c'est que le panier est vide
